...
public class StockCountGainController {
    ...
    public StockCountGainController(StockCountGainAppService appService, DocumentLockService lockService) {
    ...
    public Map<String, Object> saveDraft(@RequestBody StockCountGainDraftRequest request) {
        lockService.assertWritable("stockCountGain", request.billNo());
        var result = appService.saveDraft(request);
        lockService.releaseIfOwned("stockCountGain", String.valueOf(result.getOrDefault("billNo", request.billNo())));
        return result;
    }
