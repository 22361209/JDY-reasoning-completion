...
@Service
public class StockCountGainAppService {
    private static final String BILL_TABLE = "stock_count_gain";
    private static final BillLifecycleTarget LIFECYCLE_TARGET = new BillLifecycleTarget(BILL_TABLE, "stock_count_gain_line", "bill_id", "INVENTORY", "stock_count_gain");
    ...
    public StockCountGainAppService(JdbcTemplate jdbcTemplate, LookupService lookupService, ValidationService validationService, BillLifecycleService lifecycleService, PostingPipeline postingPipeline, NumberingService numberingService, ProductSnapshotService productSnapshotService) {
    ...
    public Map<String, Object> detail(String billNo) {
        var billRows = jdbcTemplate.queryForList("""
            ...
            FROM stock_count_gain b
            LEFT JOIN stock_count sc ON sc.id = b.source_bill_id
            WHERE b.bill_no = ?
            """, billNo);
        if (billRows.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "盘盈单不存在");
        }
        var lines = jdbcTemplate.queryForList("""
            ...
            FROM stock_count_gain_line l
            JOIN md_product p ON p.id = l.product_id
            JOIN md_warehouse w ON w.id = l.warehouse_id
            JOIN stock_count_gain b ON b.id = l.bill_id
            WHERE b.bill_no = ?
            ORDER BY l.line_no
            """, billNo);
    ...
    @Transactional
    public Map<String, Object> saveDraft(StockCountGainDraftRequest request) {
        var billNo = numberingService.assignBillNo("stockCountGain", request.billNo());
        ...
        var bill = jdbcTemplate.queryForMap("""
            INSERT INTO stock_count_gain (bill_no, bill_date, department, document_type, business_type, status, total_amount, owner_name)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ...
            SET bill_date = EXCLUDED.bill_date,
                ...
                updated_at = now(),
                version = stock_count_gain.version + 1
            WHERE stock_count_gain.status = ?
        ...
        jdbcTemplate.update("DELETE FROM stock_count_gain_line WHERE bill_id = ?::uuid", billId);
    ...
    public Map<String, Object> audit(String billNo) {
        var row = lifecycleService.transition(BILL_TABLE, billNo, BillStatus.DRAFT, BillStatus.AUDITED,
            "id::text AS id, bill_no AS \"billNo\", status", "INVENTORY", "AUDIT", "stock_count_gain", "盘盈单不存在或已审核");
        for (var line : postingLines(billNo)) {
            postingPipeline.post(new PostingContext(InventoryPostingHook.CHANNEL, String.valueOf(line.get("productCode")), String.valueOf(line.get("warehouseCode")), (BigDecimal) line.get("qty"), "STOCK_COUNT_GAIN", "STOCK_COUNT_GAIN:" + billNo));
        }
        return row;
    }
    ...
    public Map<String, Object> reverse(String billNo) {
        var row = lifecycleService.transition(BILL_TABLE, billNo, BillStatus.AUDITED, BillStatus.DRAFT,
            "id::text AS id, bill_no AS \"billNo\", status", "INVENTORY", "REVERSE", "stock_count_gain", "盘盈单不存在或不能反审核");
        for (var line : postingLines(billNo)) {
            postingPipeline.post(new PostingContext(InventoryPostingHook.CHANNEL, String.valueOf(line.get("productCode")), String.valueOf(line.get("warehouseCode")), ((BigDecimal) line.get("qty")).negate(), "STOCK_COUNT_GAIN_REVERSE", "STOCK_COUNT_GAIN_REVERSE:" + billNo));
        }
        return row;
    }
    ...
    private void insertLines(Object billId, List<StockCountGainLineRequest> lines) {
        ...
        for (var line : lines) {
            ...
            jdbcTemplate.update("""
                INSERT INTO stock_count_gain_line (bill_id, line_no, product_id, product_code_snapshot, product_name_snapshot, product_spec_snapshot, warehouse_id, source_line_no, qty, unit_price, amount, line_remark)
                VALUES (?::uuid, ?, ?::uuid, ?, ?, ?, ?::uuid, ?, ?, ?, ?, ?)
                """, billId, lineNo, product.id(), product.code(), product.name(), product.spec(), warehouseId, line.sourceLineNo(), qty, unitPrice, qty.multiply(unitPrice), validationService.optionalText(line.lineRemark()));
            lineNo += 1;
        }
    }
    ...
    public record StockCountGainDraftRequest(String billNo, String supplierCode, String billDate, String department, String ownerName, List<StockCountGainLineRequest> lines) {
        public StockCountGainDraftRequest {
    ...
    public record StockCountGainLineRequest(String productId, String productCode, String warehouseCode, Integer sourceLineNo, BigDecimal qty, BigDecimal unitPrice, String lineRemark) {
    }
}
