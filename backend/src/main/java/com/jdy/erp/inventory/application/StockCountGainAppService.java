...
public class StockCountGainAppService {
    ...
    public Map<String, Object> saveDraft(StockCountGainDraftRequest request) {
        ...
        if (bills.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "只有草稿盘盈单可以覆盖保存");
        }
        var bill = bills.getFirst();
        var billId = bill.get("id");
        inventoryTraceLifecycleService.prepareForLineReplacement("STOCK_COUNT_GAIN", billId);
    ...
    public Map<String, Object> audit(String billNo) {
        ...
        for (var line : postingLines(billNo)) {
            postingPipeline.post(PostingContext.inventory(InventoryPostingCommand.document(
                ...
                (BigDecimal) line.get("qty"), "STOCK_COUNT_GAIN", "STOCK_COUNT_GAIN",
    ...
    public Map<String, Object> reverse(String billNo) {
        ...
        for (var line : postingLines(billNo)) {
            postingPipeline.post(PostingContext.inventory(InventoryPostingCommand.document(
                String.valueOf(line.get("productCode")), String.valueOf(line.get("warehouseCode")),
                ((BigDecimal) line.get("qty")).negate(), "STOCK_COUNT_GAIN_REVERSE", "STOCK_COUNT_GAIN",
                line.get("sourceBillId"), line.get("sourceBillLineId"), billNo,
