...
public class OtherStockInAppService {
    ...
    public Map<String, Object> detail(String billNo) {
        var billRows = jdbcTemplate.queryForList("""
            ...
            """, billNo);
        if (billRows.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "其他入库单不存在");
        }
        var lines = jdbcTemplate.queryForList("""
            ...
            FROM other_stock_in_line l
            JOIN md_product p ON p.id = l.product_id
            JOIN md_warehouse w ON w.id = l.warehouse_id
            JOIN other_stock_in b ON b.id = l.bill_id
            WHERE b.bill_no = ?
            ORDER BY l.line_no
            """, billNo);
    ...
    public Map<String, Object> saveDraft(OtherStockInDraftRequest request) {
        ...
        var bills = jdbcTemplate.queryForList("""
            INSERT INTO other_stock_in (bill_no, bill_date, department, business_type, status, total_amount, owner_name)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ...
            SET bill_date = EXCLUDED.bill_date,
                ...
                updated_at = now(),
                version = other_stock_in.version + 1
            WHERE other_stock_in.status = ?
        ...
        if (bills.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "只有草稿其他入库单可以覆盖保存");
        }
        var bill = bills.getFirst();
        var billId = bill.get("id");
        inventoryTraceLifecycleService.prepareForLineReplacement("OTHER_STOCK_IN", billId);
        jdbcTemplate.update("DELETE FROM other_stock_in_line WHERE bill_id = ?::uuid", billId);
    ...
    private void insertLines(Object billId, List<OtherStockInLineRequest> lines) {
        ...
        for (var line : lines) {
            ...
            jdbcTemplate.update("""
                INSERT INTO other_stock_in_line (bill_id, line_no, product_id, product_code_snapshot, product_name_snapshot, product_spec_snapshot, warehouse_id, qty, unit_price, amount, line_remark)
    ...
    public record OtherStockInDraftRequest(String billNo, String supplierCode, String billDate, String department, String ownerName, List<OtherStockInLineRequest> lines) {
        public OtherStockInDraftRequest {
    ...
    public record OtherStockInLineRequest(String productId, String productCode, String warehouseCode, Integer sourceLineNo, BigDecimal qty, BigDecimal unitPrice, String lineRemark) {
    }
}
