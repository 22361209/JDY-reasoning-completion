...
- 物料是本阶段核心，按云星辰物料页收敛为基本信息、商品特性、价格设置、库存预警、生产信息和其他信息；保留“一物料多业务面”：可采购、可销售、可库存、可自制、可委外、默认仓库、默认供应商、默认领料仓、默认生产车间等。
- 物料类别和计量单位作为独立轻主数据维护，物料建档引用它们；多单位换算、条码明细表、图片附件和复杂库存策略后置。
