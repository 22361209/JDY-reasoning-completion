...
> 本文保留 A115 时期的元数据驱动计划与设计背景。它不是当前任务队列；若未来需要继续或调整该方向，必须建立新的产品任务包并重新评估范围、现状和验收。
...
- 采购退货单 `sourcePolicy=purchaseIn`，从已审核采购入库单选源；已审核采购退货占用源采购入库行可退数量，草稿不产生业务事实。
...
- 采购核心回归通过。
- 采购入库选源单、已审核下游占用采购订单剩余量仍有效；草稿/未审核不扣减正式可入库量，审核时由生命周期统一汇总守卫拦截超量。
...
- 物料是本阶段核心，按云星辰物料页收敛为基本信息、商品特性、价格设置、库存预警、生产信息、其他信息和附件；保留“一物料多业务面”：可采购、可销售、可库存、可自制、可委外、默认仓库、默认供应商、默认生产车间等。默认领料仓和发料方式先不作为物料表头能力暴露。
- 物料类别和计量单位作为独立轻主数据维护，物料建档引用它们；多单位换算、条码明细表和复杂库存策略后置。附件第一版支持物料图纸 PDF 与图片文件记录，后续再替换成独立附件服务。
- 主数据定义必须显性区分隐藏 UUID、只读系统编号和业务编码：底层关联使用 UUID；物料、客户、供应商、仓库列表/建档页显示只读系统编号；选择器、搜索、打印仍以业务编码和名称为主，避免把技术主键暴露成业务录入字段。
- 核心单据采用物料显示快照方案 B：新单据保存时落库物料编码、名称、规格等快照；历史列表、详情和打印优先显示快照；点击追溯时再通过 UUID 打开当前物料主档，页面可同时显示系统编号辅助人工核对。
