...
物料选择不是单据页面的临时商品弹窗，而是 `product-master-list` 的选择态。它和选源单的关系如下：

- 选源单弹窗 = 单据明细列表的选择态。
- 物料选择弹窗 = 物料资料列表的选择态。

因此物料资料列表是正本，物料选择弹窗只能派生自正本能力：

- `product-master-list` 必须具备左侧物料类别分类器和右侧共享列表表格，分类器点击后通过列表查询协议传入 `category` 条件。
- 物料选择弹窗必须复用同一套物料类别分类器、同一套 `product-master-list` 查询、同一套表格网格线和列宽/筛选/分页协议。
- 物料选择弹窗默认列可以使用 `MasterDataDefinition.selectorColumns`，但字段来源、筛选语义和类别过滤必须来自 `product-master-list`，不得另造商品候选模型。
- 所有分录表打开的物料选择入口必须统一落到这一个物料选择态；销售报价单、销售订单、发货通知单、采购订单、采购入库、采购退货、库存/生产单据和销售出库不得各写一套商品弹窗。
- BOM、委外等专用表体如暂未使用弹窗，必须登记为非弹窗物料查找点；一旦改为整列表选择，必须接入同一物料选择态。

物料列表/弹窗的表格网格线只能由真实单元格边框负责；选择态不得叠加列表 body 背景横线，避免真实行边框与背景网格错位。
