    ...
    for (const warehouseCode of ["CK-001", "CK-002", "CK-T413874"]) {
      await requireApi("/api/inventory/adjustments", {
        body: {
          ...
          warehouseCode,
          qtyDelta: 2000,
          txnType: "A21_PUSH_CONFIRM_BIDIRECTIONAL_IN",
          sourceBillType: `A21_PUSH_CONFIRM_BIDIRECTIONAL:${batch}`
        }
      });
    }
  }
...
async function createData() {
  await seedStock();
  const salesOrderNo = `XSDD-A21-${batch}`;
  const firstSalesOutNo = `XSCK-A21-PART-${batch}`;
  ...
  const purchaseOrderNo = `CGDD-A21-${batch}`;
  const firstPurchaseInNo = `CGRK-A21-PART-${batch}`;
...
async function exercisePushConfirm(page, flow) {
  ...
  assertArray(`${flow.name} default remaining`, defaults, [6, 5, 4]);
  ...
  assertArray(`${flow.name} no selection applies all`, allHalf, [3, 2.5, 2]);
  ...
  assertArray(`${flow.name} selected ratio only changes row 2`, selectedHalf, [6, 2.5, 4]);
  ...
  if (!selectionSummary.includes("已选 1 行")) {
    throw new Error(`${flow.name} selection summary mismatch: ${selectionSummary}`);
  }
  ...
  assertArray(`${flow.name} form qtys`, formQtys, [6, 2.5, 4]);
...
try {
  ...
  const sales = await exercisePushConfirm(page, {
    name: "sales",
    moduleName: "销售管理",
    ...
    screenshotPrefix: "a21-sales-push-confirm-selected"
  });

  await page.goto(frontendUrl, { waitUntil: "networkidle" });
  const purchase = await exercisePushConfirm(page, {
    name: "purchase",
    moduleName: "采购管理",
    ...
    screenshotPrefix: "a21-purchase-push-confirm-selected"
  });
