...
const baselinePath = path.join(rootDir, "verification/a2-a103-regression-latest.json");
const resultPath = path.join(rootDir, "verification/a2-a104-regression-latest.json");
...
const summary = {
  ...
  expectedMinimum: baseline.expectedTotal + appendedScripts.length,
  expectedTotal: scripts.length,
  badCount: bad.length,
  skipped: scripts.length - results.length,
  startedAt,
  finishedAt: new Date().toISOString(),
  note: "A104 full suite: A2-A103 baseline plus backend document edit lock and readonly/override regression.",
  results
};
...
console.log(JSON.stringify({
  ...
  resultPath: "verification/a2-a104-regression-latest.json"
}, null, 2));
