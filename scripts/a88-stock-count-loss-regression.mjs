...
const fixtureTag = `A88-STOCK-COUNT-LOSS-${runId}`;
const warehouseCode = `A88L-${compactRunId}`;
const warehouseName = `A88 盘亏仓 ${runId}`;
const seedSourceType = `A88_LOSS_SEED:${compactRunId}`;
const seedTxnType = "A88_LOSS_SEED";
const shortageFailureEndpoint = "POST /api/stock-count-losses/{billNo}/audit";
const shortageFailureReason = "库存不足，不能调整为负数";
const fixtureUsername = `a88l_${compactRunId.toLowerCase()}`;
const fixturePassword = `A88-${compactRunId.slice(-8)}-Admin!`;
const fixtureDisplayName = `A88 盘亏管理员 ${runId}`;
...
const documentType = "stockCountLoss";
const faultPhase = process.env.A88_STOCK_COUNT_LOSS_FAULT_PHASE ?? "";
...
const result = {
  ...
  seed: null,
  shortage: null,
  artifacts: { fixtureUsername, fixtureUserId: null, fixtureGrantId: null, adminRoleId: null, shortageFailureLogId: null },
  screenshots: [],
...
const seedSourceBillLineId = javaNameUuid(`TEST_LINE:${seedSourceType}:${productCode}:${warehouseCode}`);
...
function runDocuments() {
  return dbJson(`
    ...
    FROM public.stock_count_loss header
    JOIN public.stock_count_loss_line line ON line.bill_id=header.id
...
function transactionsForDocument(headerId) {
  return dbJson(`
    ...
    WHERE txn.account_set_id=${sqlLiteral(accountSetId)}::uuid
      AND txn.source_bill_type='STOCK_COUNT_LOSS'
...
function capture(rows) {
  for (const row of rows) {
    assert(uuidPattern.test(row.id), "owned loss transaction must expose a UUID", row);
    capturedTxnIds.add(row.id);
  }
  return rows;
}

function assertSeedTransaction(row, warehouseId) {
  assert(row.accountSetId === accountSetId && row.productId === productId, "loss seed must keep exact account/product identity", row);
  assert(row.warehouseId === warehouseId && row.warehouseCode === warehouseCode, "loss seed must belong only to the created warehouse UUID", row);
  assert(row.txnType === seedTxnType && row.sourceBillType === seedSourceType, "loss seed must keep its exact A-number TEST identity", row);
  assert(row.sourceBillId === seedSourceBillId && row.sourceBillLineId === seedSourceBillLineId, "loss seed must keep deterministic TEST header/line UUIDs", row);
  assert(row.sourceBillNo === seedSourceType && row.sourceBillDate === seedSourceBillDate, "loss seed must keep exact source number/date", row);
  assert(row.traceQuality === "TEST" && row.postingAction === "AUDIT" && row.reversalOfTxnId === null, "loss seed must remain a TEST/AUDIT fact without reversal", row);
  assert(Number(row.qtyDelta) === seedQty, "loss seed quantity mismatch", row);
}

function assertLossTrace(rows, document, reversed) {
  const expected = reversed
    ? [
        { txnType: "STOCK_COUNT_LOSS", qtyDelta: -qty, action: "AUDIT", reverseOf: null },
        { txnType: "STOCK_COUNT_LOSS_REVERSE", qtyDelta: qty, action: "REVERSE", reverseOf: "STOCK_COUNT_LOSS" }
      ]
    : [{ txnType: "STOCK_COUNT_LOSS", qtyDelta: -qty, action: "AUDIT", reverseOf: null }];
  assert(rows.length === expected.length, `stock count loss should expose ${expected.length} exact owned transactions`, rows);
  for (const spec of expected) {
    ...
    assert(row.sourceBillType === "STOCK_COUNT_LOSS", `${spec.txnType} must keep canonical source type`, row);
...
async function assertHealthBeforeSession() {
  assert(new URL(apiBase).hostname === "127.0.0.1" && new URL(frontendUrl).hostname === "127.0.0.1", "A88 loss may only write through local services");
  ...
  assert(health.body?.testInventoryAdjustmentApi === true, "A88 loss requires the restricted test inventory adjustment capability", health.body);
...
async function assertAuthenticatedEnvironmentBeforeWrites() {
  ...
  assert(session.body?.tenant?.code === "BLD-TEST", "A88 loss writes are allowed only in BLD-TEST", session.body?.tenant);
  assert(session.body?.tenant?.schemaName === "public", "A88 loss direct SQL is allowed only in public", session.body?.tenant);
  ...
  assert(authenticatedProductId === productId, "A88 loss authenticated route must retain the prepared product UUID", { productCode, productId, authenticatedProductId });
  result.environment.tenant = session.body.tenant;
  result.environment.productId = productId;
}

function prepareFixtureRoute() {
  const route = dbJson(`SELECT jsonb_build_object('accountSetId', account_set.id::text, 'accountSetCode', account_set.code,
    'schemaName', account_set.schema_name, 'accountEnabled', account_set.enabled, 'adminRoleId', role.id::text, 'roleEnabled', role.enabled)::text
    FROM public.sys_account_set account_set CROSS JOIN public.sys_role role WHERE account_set.code='BLD-TEST' AND role.code='ADMIN'`);
  ...
  assert(uuidPattern.test(productId), "A88 loss requires an enabled run product UUID before fixture identity writes", { productCode, productId });
}

function setupFixtureAdminUser() {
  assert(uuidPattern.test(accountSetId) && uuidPattern.test(adminRoleId) && uuidPattern.test(productId), "fixture route must be prepared before creating loss identity");
...
function setupWarehouse() {
  ...
  assert(collisions.length === 0, "run-unique loss warehouse must not pre-exist", collisions);
  ...
  `) ?? [];
  assert(inserted.length === 1 && uuidPattern.test(inserted[0].id) && inserted[0].remark === fixtureTag, "one run-unique loss warehouse must be inserted", inserted);
  createdWarehouseIds.add(inserted[0].id);
  const owned = ownedWarehouseRows();
  assert(owned.length === 1 && owned[0].code === warehouseCode && owned[0].name === warehouseName && owned[0].remark === fixtureTag, "created loss warehouse must retain exact ownership", owned);
}

async function seedOnlyRunWarehouse() {
  ...
  assert(rows.length === 1, "loss seed must create exactly one owned transaction", rows);
  const warehouse = ownedWarehouseRows()[0];
  assert(warehouse, "created loss warehouse UUID must remain available for seed validation", ownedWarehouseRows());
  assertSeedTransaction(rows[0], warehouse.id);
  result.seed = rows[0];
  assert(Number(stockState().onHand) === seedQty && Number(stockState().available) === seedQty, "loss seed stock state mismatch", stockState());
}
...
async function createAndAuditInFrontend() {
  ...
  try {
    ...
    assert(await billNoInput.inputValue() === "", "new stock count loss bill number should be blank");
    assert(!(await billNoInput.isEditable()), "new stock count loss bill number should be readonly");
    await page.getByTestId("stock-count-loss-bill-date").fill(billDate);
    await page.getByTestId("stock-count-loss-department").fill(fixtureTag);
    ...
    assert(/^PK\d{6}$/.test(billNo), "saved loss bill number must be generated", billNo);
    await auditDocument(page);
    await page.getByTestId("document-status").filter({ hasText: "已审核" }).waitFor({ state: "visible" });
    const formShot = `a88-stock-count-loss-form-audited-${runId}.png`;
    ...
    const listShot = `a88-stock-count-loss-list-${runId}.png`;
...
async function runScenario() {
  setupWarehouse();
  await seedOnlyRunWarehouse();
  if (faultPhase === "after-seed") {
    throw new Error("A88 injected loss failure after unique-warehouse seed");
  }
  const before = stockState();
  result.screenshots = await createAndAuditInFrontend();
  const document = documentByBillNo(billNo);
  assert(document && uuidPattern.test(document.headerId) && uuidPattern.test(document.lineId), "saved loss must expose exact header and line UUIDs", document);
  assert(document.billDate === billDate, "saved loss business date mismatch", document);
  ...
  assertLossTrace(auditRows, document, false);
  const afterAudit = stockState();
  assert(detailAfterAudit.document.status === "AUDITED", "loss must be audited", detailAfterAudit.document);
  assert(Number(afterAudit.onHand) === Number(before.onHand) - qty, "loss audit stock delta mismatch", { before, afterAudit });
  ...
  assertLossTrace(reverseRows, document, true);
  const afterReverse = stockState();
  assert(detailAfterReverse.document.status === "DRAFT", "reversed loss must return to DRAFT", detailAfterReverse.document);
  assert(Number(afterReverse.onHand) === Number(before.onHand), "loss reverse must restore seeded stock", { before, afterReverse });

  const shortageDraft = await api("/api/stock-count-losses/draft", {
    body: {
      billNo: "",
      billDate,
      ...
      lines: [{
        productCode,
        warehouseCode,
        qty: Math.max(Number(afterReverse.onHand), Number(afterReverse.available)) + 100000,
        unitPrice,
  ...
  assert(/^PK\d{6}$/.test(shortageBillNo), "shortage loss draft must expose a generated bill number", shortageDraft.body);
  const shortageDocument = documentByBillNo(shortageBillNo);
  assert(shortageDocument && uuidPattern.test(shortageDocument.headerId) && uuidPattern.test(shortageDocument.lineId), "shortage loss draft must expose exact header and line UUIDs", shortageDocument);
  const beforeShortage = stockState();
  ...
  assert(newFailureLogs.length === 1, "shortage loss must create exactly one new WRITE_FAILED/http_endpoint log UUID", newFailureLogs);
  ...
  assert(shortageResponse.status === 409, "shortage loss audit must fail with 409", shortageResponse);
  assert(shortageReason === shortageFailureReason, "shortage loss must report the formal nonnegative-stock guard", shortageResponse.body);
  assert(JSON.stringify(stockState()) === JSON.stringify(beforeShortage), "shortage loss audit must not mutate stock");
  assert(transactionsForDocument(shortageDocument.headerId).length === 0, "shortage loss audit must not create inventory facts");
  ...
  `);
  assert(financeCount === 0, "stock count loss must not create finance documents", financeCount);
  result.stock = { before, afterAudit, afterReverse, beforeShortage };
  result.transactions = [...seedTransactions(), ...reverseRows];
  result.shortage = { status: shortageResponse.status, reason: shortageReason, endpoint: shortageFailureEndpoint,
    failureLogId: shortageFailureLog.id, failureLog: shortageFailureLog };
}
...
function fixtureUserState() {
  ...
  "cleanup refused a fixture loss admin without exact UUID/identity", users);
  ...
  "cleanup refused a fixture loss role outside exact ADMIN link", roleLinks);
  ...
  "cleanup refused a fixture loss grant outside exact BLD-TEST ADMIN grant", grants);
  ...
  for (const scope of sessionScopes) {
    ...
    "cleanup refused a loss session scope outside the exact fixture actor/account", scope);
  }
...
function discoverCleanupState() {
  ...
  "cleanup refused a loss warehouse without exact UUID/fixture ownership", warehouses);
  ...
  const headers = dbJson(`SELECT COALESCE(jsonb_agg(to_jsonb(header) ORDER BY header.id), '[]'::jsonb)::text
    FROM public.stock_count_loss header
    WHERE header.department=${sqlLiteral(fixtureTag)}
       OR header.id IN (SELECT line.bill_id FROM public.stock_count_loss_line line WHERE line.warehouse_id=ANY(${uuidArray(warehouseIds)}))`) ?? [];
  assert(headers.length <= 2 && headers.every((row) => uuidPattern.test(String(row.id)) && /^PK\d{6}$/.test(row.bill_no)
    && row.bill_date === billDate && row.department === fixtureTag && row.document_type === "STK_StockCountLoss"
    && row.business_type === "盘亏单"), "cleanup refused a loss header outside the exact fixture closure", headers);
  ...
  const lines = dbJson(`SELECT COALESCE(jsonb_agg(to_jsonb(line) ORDER BY line.id), '[]'::jsonb)::text
    FROM public.stock_count_loss_line line
  ...
  assert(lines.length === headers.length && lines.every((row) => uuidPattern.test(String(row.id))
    ...
    && ownedWarehouseIdSet.has(String(row.warehouse_id)) && Number(row.qty) > 0),
  "cleanup refused a loss line outside the exact current document closure", lines);
  ...
  for (const txn of transactions) {
    assert(String(txn.account_set_id) === accountSetId && String(txn.product_id) === productId
      && ownedWarehouseIdSet.has(String(txn.warehouse_id)), "cleanup refused a foreign loss transaction scope", txn);
    if (txn.source_bill_type === seedSourceType) {
      assert(uuidPattern.test(String(txn.id)) && txn.txn_type === seedTxnType
        && String(txn.source_bill_id) === seedSourceBillId && String(txn.source_bill_line_id) === seedSourceBillLineId
        && txn.source_bill_no === seedSourceType && txn.source_bill_date === seedSourceBillDate
        && txn.trace_quality === "TEST" && txn.posting_action === "AUDIT" && txn.reversal_of_txn_id === null
        && Number(txn.qty_delta) === seedQty, "cleanup refused a changed loss TEST seed fact", txn);
      seedRows.push(txn);
      capturedTxnIds.add(String(txn.id));
      continue;
    }
    const sourceLine = lineById.get(String(txn.source_bill_line_id));
    const isAudit = txn.txn_type === "STOCK_COUNT_LOSS" && txn.posting_action === "AUDIT"
      && Number(txn.qty_delta) === -Number(sourceLine?.qty) && txn.reversal_of_txn_id === null;
    const isReverse = txn.txn_type === "STOCK_COUNT_LOSS_REVERSE" && txn.posting_action === "REVERSE"
      && Number(txn.qty_delta) === Number(sourceLine?.qty);
    assert(txn.source_bill_type === "STOCK_COUNT_LOSS" && sourceLine && String(sourceLine.bill_id) === String(txn.source_bill_id)
      ...
      && txn.source_bill_date === billDate && txn.trace_quality === "EXACT" && (isAudit || isReverse),
    "cleanup refused a loss fact outside the exact current header/line/source/action closure", { txn, sourceLine });
    formalRows.push(txn);
    capturedTxnIds.add(String(txn.id));
  }
  assert(seedRows.length <= 1, "cleanup refused duplicate loss TEST seed facts", seedRows);
  for (const line of lines) {
    const rows = formalRows.filter((txn) => String(txn.source_bill_line_id) === String(line.id));
    const auditRows = rows.filter((txn) => txn.txn_type === "STOCK_COUNT_LOSS" && txn.posting_action === "AUDIT");
    const reverseRows = rows.filter((txn) => txn.txn_type === "STOCK_COUNT_LOSS_REVERSE" && txn.posting_action === "REVERSE");
    assert((rows.length === 0 || rows.length === 1 || rows.length === 2) && auditRows.length === (rows.length === 0 ? 0 : 1)
      && reverseRows.length === (rows.length === 2 ? 1 : 0), "cleanup refused an incomplete or duplicate loss posting set", rows);
    if (reverseRows.length === 1) assert(String(reverseRows[0].reversal_of_txn_id) === String(auditRows[0].id), "loss reverse must pair with exact same-line audit fact", rows);
  }
  ...
  assert(balances.every((row) => String(row.account_set_id) === accountSetId && String(row.product_id) === productId
    && ownedWarehouseIdSet.has(String(row.warehouse_id))), "cleanup refused a foreign loss balance", balances);
  ...
  for (const log of logs) {
    assert(uuidPattern.test(String(log.id)) && String(log.operated_by ?? "") === fixtureUserId
      ...
      "cleanup refused an operation log outside the exact loss fixture actor", log);
    capturedLogIds.add(String(log.id));
  }
  const failureLogs = shortageFailureLog === null ? [] : logs.filter((row) => String(row.id) === String(shortageFailureLog.id));
  ...
  for (const log of failureLogs) {
    assert(String(log.id) === shortageFailureLog.id && log.module_code === "SECURITY" && log.action_code === "WRITE_FAILED"
      && log.target_type === "http_endpoint" && log.target_id === null && log.target_no === shortageFailureEndpoint
      && log.success === false && log.failure_reason === shortageFailureReason && String(log.account_set_id) === accountSetId
      && log.account_set_code === "BLD-TEST" && log.actor_type === "USER" && log.actor_username === fixtureUsername,
    "cleanup shortage failure log changed", { log, shortageFailureLog });
  }
  ...
  for (const lock of locks) {
    ...
    "cleanup refused a loss document lock outside the exact fixture actor/document", lock);
  }
  return {
    fixtureUser, warehouses, headers, lines, transactions, balances, logs, locks, failureLogs,
...
function cleanupFixtures() {
  ...
  assert(txnIds.every((id) => capturedTxnIds.has(id)), "all loss cleanup inventory facts must have captured UUIDs", { txnIds, captured: result.cleanup.capturedTxnIds });
  ...
  dbScalar(`
    ...
    LOCK TABLE public.sys_user, public.sys_role, public.sys_user_role, public.sys_account_set,
      ...
      public.stock_count_loss, public.stock_count_loss_line, public.inv_stock_txn,
      public.inv_stock_balance, public.sys_operation_log, public.doc_edit_lock
      IN SHARE ROW EXCLUSIVE MODE;
    DO $a88_stock_count_loss_cleanup$
    DECLARE
      actual jsonb;
    BEGIN
      ...
      IF actual IS DISTINCT FROM ${sqlLiteral(JSON.stringify(state.fixtureUser.users))}::jsonb THEN RAISE EXCEPTION 'A88 loss cleanup refused: user snapshot changed'; END IF;
      ...
      IF actual IS DISTINCT FROM ${sqlLiteral(JSON.stringify(state.fixtureUser.roleLinks))}::jsonb THEN RAISE EXCEPTION 'A88 loss cleanup refused: role snapshot changed'; END IF;
      ...
      IF actual IS DISTINCT FROM ${sqlLiteral(JSON.stringify(state.fixtureUser.grants))}::jsonb THEN RAISE EXCEPTION 'A88 loss cleanup refused: grant snapshot changed'; END IF;
      ...
      IF actual IS DISTINCT FROM ${sqlLiteral(JSON.stringify(state.fixtureUser.sessionScopes))}::jsonb THEN RAISE EXCEPTION 'A88 loss cleanup refused: session scope snapshot changed'; END IF;
      ...
      WHERE warehouse_row.id=ANY(${uuidArray([...createdWarehouseIds])}) OR warehouse_row.code=${sqlLiteral(warehouseCode)};
      IF actual IS DISTINCT FROM ${sqlLiteral(JSON.stringify(state.warehouses))}::jsonb THEN RAISE EXCEPTION 'A88 loss cleanup refused: warehouse snapshot changed'; END IF;
      SELECT COALESCE(jsonb_agg(to_jsonb(header) ORDER BY header.id), '[]'::jsonb) INTO actual FROM public.stock_count_loss header
      WHERE header.department=${sqlLiteral(fixtureTag)} OR header.id IN (SELECT line.bill_id FROM public.stock_count_loss_line line WHERE line.warehouse_id=ANY(${uuidArray(warehouseIds)}));
      IF actual IS DISTINCT FROM ${sqlLiteral(JSON.stringify(state.headers))}::jsonb THEN RAISE EXCEPTION 'A88 loss cleanup refused: header snapshot changed'; END IF;
      SELECT COALESCE(jsonb_agg(to_jsonb(line) ORDER BY line.id), '[]'::jsonb) INTO actual FROM public.stock_count_loss_line line
      WHERE line.bill_id=ANY(${uuidArray(documentIds)}) OR line.warehouse_id=ANY(${uuidArray(warehouseIds)});
      IF actual IS DISTINCT FROM ${sqlLiteral(JSON.stringify(state.lines))}::jsonb THEN RAISE EXCEPTION 'A88 loss cleanup refused: line snapshot changed'; END IF;
      SELECT COALESCE(jsonb_agg(to_jsonb(txn) ORDER BY txn.id), '[]'::jsonb) INTO actual FROM public.inv_stock_txn txn
      WHERE txn.warehouse_id=ANY(${uuidArray(warehouseIds)}) OR txn.source_bill_id=ANY(${uuidArray(documentIds)});
      IF actual IS DISTINCT FROM ${sqlLiteral(JSON.stringify(state.transactions))}::jsonb THEN RAISE EXCEPTION 'A88 loss cleanup refused: transaction snapshot changed'; END IF;
      ...
      IF actual IS DISTINCT FROM ${sqlLiteral(JSON.stringify(state.balances))}::jsonb THEN RAISE EXCEPTION 'A88 loss cleanup refused: balance snapshot changed'; END IF;
      SELECT COALESCE(jsonb_agg(to_jsonb(log_row) ORDER BY log_row.id), '[]'::jsonb) INTO actual FROM public.sys_operation_log log_row
      WHERE log_row.operated_by=${sqlLiteral(fixtureUserId)}::uuid OR log_row.actor_username=${sqlLiteral(fixtureUsername)}
         OR log_row.target_id=ANY(${uuidArray(logTargetIds)}) OR log_row.target_no=ANY(${textArray(logTargetNos)});
      IF actual IS DISTINCT FROM ${sqlLiteral(JSON.stringify(state.logs))}::jsonb THEN RAISE EXCEPTION 'A88 loss cleanup refused: log snapshot changed'; END IF;
      ...
      WHERE (lock_row.document_type=${sqlLiteral(documentType)} AND lock_row.bill_no=ANY(${textArray(billNos)}))
         OR lock_row.holder_user_id=${sqlLiteral(fixtureUserId)}::uuid OR lock_row.holder_username=${sqlLiteral(fixtureUsername)};
      IF actual IS DISTINCT FROM ${sqlLiteral(JSON.stringify(state.locks))}::jsonb THEN RAISE EXCEPTION 'A88 loss cleanup refused: document lock snapshot changed'; END IF;
      ...
      DELETE FROM public.stock_count_loss_line WHERE id=ANY(${uuidArray(lineIds)});
      DELETE FROM public.stock_count_loss WHERE id=ANY(${uuidArray(documentIds)});
      ...
      IF EXISTS (SELECT 1 FROM public.stock_count_loss WHERE id=ANY(${uuidArray(documentIds)}) OR department=${sqlLiteral(fixtureTag)})
         OR EXISTS (SELECT 1 FROM public.stock_count_loss_line WHERE id=ANY(${uuidArray(lineIds)}) OR warehouse_id=ANY(${uuidArray(warehouseIds)}))
        ...
        RAISE EXCEPTION 'A88 loss cleanup closure is not zero';
      END IF;
    END;
    $a88_stock_count_loss_cleanup$;
    COMMIT;
  `);

  const residue = {
    documents: dbNumber(`SELECT count(*) FROM public.stock_count_loss WHERE department=${sqlLiteral(fixtureTag)}`),
    lines: lineIds.length ? dbNumber(`SELECT count(*) FROM public.stock_count_loss_line WHERE id=ANY(${uuidArray(lineIds)})`) : 0,
  ...
  assert(Object.values(residue).every((value) => value === 0), "A88 loss cleanup must leave zero residue", residue);
}
...
try {
  ...
  if (faultPhase === "after-identity") {
    throw new Error("A88 injected loss failure after fixture identity creation");
  }
...
if (primaryError || result.cleanup.error) {
  throw new Error(`A88 stock count loss regression failed: ${primaryError?.message ?? "scenario passed"}; cleanup: ${result.cleanup.error || "ok"}`, { cause: primaryError ?? undefined });
}
