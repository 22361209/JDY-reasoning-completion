...
const cashTransferMigrationPath = path.join(
  rootDir,
...
try {
  ...
  result.migrationGuards = {
    ...
    cashTransferManagedGuard:
      cashTransferMigrationSource.includes("CREATE TABLE cash_transfer")
      && cashTransferMigrationSource.includes("CREATE TABLE cash_transfer_fact")
  ...
  const migrationRows = sqlJson(`
    ...
    WHERE version IN ('98', '99', '100', '101', '102', '103', '104', '105', '106', '107', '108', '109', '110')
  `);
  assert(migrationRows.length === 13, `expected installed V98 through V110 rows, found ${migrationRows.length}`);
  ...
  assert(cashTransferMigration.checksum === cashTransferSourceChecksum, `V109 checksum drift: installed=${cashTransferMigration.checksum} source=${cashTransferSourceChecksum}`);
  result.cashTransferMigration = {
    ...cashTransferMigration,
  ...
  for (const schema of registeredSchemas) {
    ...
    assert(firstManagedCount === 87, `${schema} first sync returned ${firstManagedCount}, expected 87`);
    ...
    assert(secondManagedCount === 87, `${schema} second sync returned ${secondManagedCount}, expected 87`);
...
function runTopologyChecks() {
  const sql = `
    ...
    BEGIN
      ...
      IF first_count <> 87 OR second_count <> 87 THEN
